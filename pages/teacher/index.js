    ...
    const createClass = ({formName, formStudents}) => {
        getAccessToken().then((accessToken) => {
            axios.post(process.env.NEXT_PUBLIC_BACKEND_HTTP_BASE+'core/classrooms/', {
                "name": formName, "no_of_students": formStudents > 1 ? parseInt(formStudents) : 1
            }, {
                headers: {'Authorization': 'Bearer '+accessToken},
            })
            .then(res => {
            ...
            { close => (
                <div className="px-4 py-4 bg-gray-100 max-w-min shadow-lg">
                    ...
                    <label>
                        ...
                        <input className="mt-2 outline-none border-2 rounded" type="text" name="class_name" onChange={e => setFormName(e.target.value)} />
                    </label>
                    <label>
                        <p className="mt-4 font-semibold">No. of Students</p>
                        <p className="text-sm -mt-1 text-gray-500">At least 1.</p>
                        <input className="mt-2 outline-none border-2 rounded" type="number" value={formStudents.toString()} min="1" name="number_of_students" onChange={e => setFormStudents(e.target.value)} />
                    </label><br />
                    {formError && <p className="text-sm text-red-500">Class name cannot be empty.</p>}
                    <button className="mt-4 py-1 px-2 bg-blue-500 hover:bg-blue-600 text-white rounded" onClick={() => {
                        if (!formName) {
                            setFormError(true)
                        } else {
                            createClass({formName, formStudents}); close()
                        }
                    }}>Create</button>
                </div>
            )}
