...
const StudentLogin = () => {
    ...
    const signupUser = (e) => {
        ...
        console.log('ran')
        axios.post(process.env.NEXT_PUBLIC_BACKEND_HTTP_BASE+'auth/token/student_register/', {
            code, name
        }, { headers: {'Content-Type': 'application/json'}})
        .then(res => {
    ...
    return (
        <div>
            ...
            <main className="flex flex-col items-center pt-8 px-4 bg-white">
                ...
                { isLogin ? (
                    <div className="">
                        <h1 className="text-2xl sm:text-4xl my-6 font-bold text-blue-700 text-center">Login to Classroom</h1>

                        { (loginDetails !== false) && (
                            <div className="flex flex-col bg-gray-800 rounded px-4 py-4 mb-6">
                                <h3 className="text-xl font-bold text-blue-200">Sign-up Successful!</h3>
                                <p className="mt-1 text-gray-200">Please remember your login credentials below.</p>
                                <div className="flex flex-row items-center mt-6 text-gray-300 text-sm"><p>Classroom Code</p> <p className="ml-auto font-mono text-2xl font-bold text-white">{loginDetails.code}</p></div>
                                <div className="flex flex-row items-center mt-1 text-gray-300 text-sm"><p>Index</p> <p className="ml-auto font-mono text-2xl font-bold text-white">{loginDetails.index}</p></div>
                            </div>
                        )}
                        ...
                        <form onSubmit={e => {{e.preventDefault(); loginUser(e)}}} className="">
                            ...
                            { auth.loading ? (
                                ...
                                <>
                                    <button className="bg-blue-500 hover:bg-blue-600 border border-blue-500 hover:border-blue-600 text-white text-lg mt-4 py-1 px-2 rounded-md w-full">Login</button>
                                    <button
                                    ...
                                    >I don't have an index number.</button>
                                </>
                            )}
                        </form>
                    </div>
                    ...
                    <>
                        <h1 className="text-2xl sm:text-4xl my-6 font-bold text-blue-700">Sign Up for Classroom</h1>
                        ...
                        <form onSubmit={e => {{e.preventDefault; signupUser(e)}}} className="">
                            ...
                            { auth.loading ? (
                                ...
                                <>
                                    <button type="submit" className="bg-blue-500 hover:bg-blue-600 border border-blue-500 hover:border-blue-600 text-white text-lg mt-4 py-1 px-2 rounded-md w-full">Sign Up</button>
                                    <button
                                        className="focus:outline-none border border-blue-500 hover:border-blue-700 text-blue-500 hover:text-blue-700 text-lg mt-4 py-1 px-2 rounded-md w-full"
                                        type="button" onClick={() => setIsLogin(!isLogin)}
                                    >I already have an index number.</button>
                                </>
                            )}
                        </form>
                    </>
