...
const Announcements = ({ classroom, announcements, setAnnouncements }) => {
  ...
  const deleteAnnouncement = (id) => {
    try {
      const existingAnnouncement = announcements.filter(
        (subAnnouncement) => subAnnouncement.id === id
      );
      const existingIndex = Object.keys(announcements).find(
        (key) => announcements[key] === existingAnnouncement[0]
      );
          ...
          .then((res) => {
            setAnnouncements([
              ...announcements.filter(
                (subAnnouncement) =>
                  Object.keys(announcements).find(
                    (key) => announcements[key] === subAnnouncement
                  ) < existingIndex
              ),
              res.data,
              ...announcements.filter(
      ...
      {(close) => (
        ...
        >
          ...
          <input
            ...
            autoComplete="off"
          />
          <label htmlFor="description" className="mt-4 mb-2 mx-2">
            Body:
          </label>
          <textarea
            onChange={(e) =>
