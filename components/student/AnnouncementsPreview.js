...
const AnnouncementsPreview = ({ announcements }) => {
    return (
        <>  <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-semibold bg-white rounded-2xl">Announcements</h1>
        </div>
        ...
        <CustomPopup
            trigger={
                <div className="mt-4 bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-lg cursor-pointer">
                    <h3 className="font-semibold text-lg text-blue-600 truncate">{announcement.name}</h3>
            ...
            {close => (
                <div className="px-4 py-4 bg-white rounded-lg shadow-lg popup ">
                    <div className="flex items-center justify-between">
                        <h1 className="my-2 mx-2 text-2xl font-bold">{announcement.name}</h1>
                        <p className="mx-2 text-gray-600">{beautifyDate(announcement.updated_at.slice(0, 10))}</p>
                    </div>
                    <div className="w-full h-0.5 bg-gray-200"></div>
                    <CustomLinkify><p className="my-4 mx-2">{announcement.description}</p></CustomLinkify>
                    <div className="mx-2 w-full h-0.5 bg-gray-200"></div>
