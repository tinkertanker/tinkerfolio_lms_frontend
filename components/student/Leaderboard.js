        ...
        <CustomPopup
            trigger={
                <div className="flex items-center justify-between bg-blue-600 hover:bg-blue-700 py-5 sm:px-6 rounded-2xl shadow-lg cursor-pointer h-full min-h-full">
                    {classroom && <h1 className="text-3xl font-bold pl-3 sm:pl-5 text-white ">{classroom.name}</h1>}
                    ...
                    <div className="sm:mx-4">
                        <div className="flex items-center justify-between py-2">
                            <div className="flex items-center gap-2">
                                ...
                                <h1 className="font-bold text-2xl sm:text-3xl mx-2">Leaderboard</h1>
                            </div>
                        ...
                        {getLeaderboard().map((student, i) => (
                            <div className={`flex items-center my-3 rounded-xl justify-between ${(profile.index === student.index) ? "bg-blue-600" : "bg-gray-100"}`}>
                                <div className="flex items-center">
                                    <h2 className={`flex items-center justify-center font-semibold text-xl sm:text-2xl w-8 sm:w-12 h-8 sm:h-12 mx-4 my-2 rounded-full ${(i === 0) ? "bg-yellow-400" : (i === 1) ? "bg-gray-400" : (i === 2) ? "bg-yellow-600" : "bg-gray-100"}`}>{i + 1}</h2>
                                    <p className={`truncate w-40 sm:w-52 mr-5 ${(profile.index === student.index) && "text-white"}`}>{(student.name.length > 0) ? student.name : <span className="italic">(Unnamed)</span>}</p>
                                </div>
                                <p className={`font-medium text-xl sm:text-2xl mr-3 sm:mx-5 ${(profile.index === student.index) ? "text-white" : "text-blue-600"}`}>{student.score} ★</p>
                            </div>
                        ))}
                    </div>
                </div>
